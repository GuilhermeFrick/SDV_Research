...
from detection.paths import PARSED_DIR
...
def extract_features(df: pd.DataFrame, state: dict, mc_label: int) -> pd.DataFrame:
    ...
    out = pd.DataFrame({
        ...
        'f22_src_clientid_diversity':f22, 'label': mc_label,
    })
    return out
...
def _load_and_normalize(csv_path: Path, mc_label: int,
                        only_attack: bool,
                        target_duration: float) -> pd.DataFrame | None:
    """Lê um CSV, mantém apenas as linhas relevantes e normaliza timestamps."""
    chunks = []
    ...
    # Normalizar: mapear [t_min, t_max] → [0, target_duration]
    t_min = df['timestamp'].min()
    t_max = df['timestamp'].max()
    dur   = t_max - t_min
    if dur > 0:
        df['timestamp_norm'] = (df['timestamp'] - t_min) / dur * target_duration
    else:
...
def main(attacks: list[str], ratio_benigno: float, out_name: str):
    ...
    df_ben = _load_and_normalize(ben_path, mc_label=0,
                                 only_attack=False, target_duration=1.0)
    target_dur = df_ben['timestamp_norm'].max()
    print(f'      {len(df_ben):,} pacotes benigno · duração norm: {target_dur:.3f}')
    ...
    for atk in attacks:
        ...
        for fname, mc_label in ATTACK_FILES[atk]:
            ...
            if not p.exists():
                ...
                continue
            df_atk = _load_and_normalize(p, mc_label=mc_label,
                                         only_attack=True,
